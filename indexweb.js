...
const dashboard = {
    ...
    init(accountConfigs) {
        this.accounts = accountConfigs.map((acc, i) => ({
            ...
            totalSwaps: 0, lastSwapDir: '', lastSwapAt: 0,
    ...
    try {
        ...
        const accounts = dashboard.accounts.map(a => {
            ...
            return {
                ...
                lastSwapAt: a.lastSwapAt || 0,
                gapPerSwap: a.totalSwaps > 0 ? Math.round((Date.now() - a.startTime) / a.totalSwaps / 1000) : 0,
...
async function performSwap(ctx, holdings) {
    ...
    async function doSwapStep(stepNum, fromPair, toPair, amount) {
        ...
        if (!isRescueSwap) {
            dashboard.update(index, { cc: ccBalance, usdcx: uBal, ceth: cBal, totalSwaps: totalSwaps + 1, lastSwapAt: Date.now() });
            totalSwaps++;
        } else {
